...
-(void)SPPstartADV{
    ...
    [PM startAdvertising:@{ CBAdvertisementDataLocalNameKey:@"LCD", CBAdvertisementDataServiceUUIDsKey :@[SPPService.UUID]}];
...
-(void)peripheralManager:(CBPeripheralManager *)peripheral didReceiveWriteRequests:(NSArray *)requests
{
    ...
    for(CBATTRequest* obj in requests){
        ...
        if([self compareCBUUIDToInt:obj.characteristic.UUID UUID2: 0xba01] ){
            ...
            NSString* Data = [[NSString alloc] initWithData:Rx.value encoding:NSUTF8StringEncoding];
            
            
            [sppDelegate SPP_Rx:Data];
